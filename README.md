...
1. 「直感」を「数値」に変える多次元レコメンドエンジン

   既存のコーヒー管理アプリの多くは「備忘録」に留まっており、「次に何を試すべきか」というユーザーの意思決定を支援する機能が不足している点に着目しました。

   課題解決: ユーザーが主観的に入力した味覚データ（酸味・苦味・コク・香り）を蓄積し、その重心を算出することで、「理想の味」を定量化しました。

   アルゴリズムの選定: 各味覚指標を $n$ 次元の座標空間として捉え、基準となる17種類の豆データとのユークリッド距離を算出するロジックを実装しました。
   成果: 数学的な裏付けに基づいた近似値を提案することで、ユーザーの潜在的な好みに合致する「次の一杯」を論理的にレコメンドする体験を実現しました。


2. Google Maps API を活用したユーザー体験の高度化
  
  「どこで買ったか」という場所の記憶は、味の記憶と密接に関係しています。単なる住所テキストの保存ではなく、地図を軸としたインターフェースを構築しました。

   動的なデータ連携: Requestsライブラリを用いて Google Geocoding API と通信し、ユーザーが入力した曖昧な住所情報から動的に緯度・経度を取得。データベースへの正規化された座標保存を自動化しました。

   双方向のデータ参照: 地図上のピンにイベントを実装し、クリック時にその店舗で購入した過去の豆リストを即座に参照できる機能を構築しました。

   こだわり: 「店舗詳細 → 購入履歴 → 各豆の評価」という動線をシームレスに繋ぐことで、ユーザーが過去の体験を直感的に振り返ることができるUXを追求しました。
